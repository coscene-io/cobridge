...
#define COS_DEBOUNCE(f, ms)                                                    \
  {                                                                            \
    ...
    if (std::chrono::duration_cast<std::chrono::milliseconds>(now - last_call) \
            .count() > ms) {                                                   \
      last_call = now;                                                         \
      f();                                                                     \
    }                                                                          \
...
constexpr auto SUBSCRIBE_PARAMETER_UPDATES =
    string_hash("subscribeParameterUpdates");
constexpr auto UNSUBSCRIBE_PARAMETER_UPDATES =
    string_hash("unsubscribeParameterUpdates");
constexpr auto SUBSCRIBE_CONNECTION_GRAPH =
    string_hash("subscribeConnectionGraph");
constexpr auto UNSUBSCRIBE_CONNECTION_GRAPH =
    string_hash("unsubscribeConnectionGraph");
...
constexpr websocketpp::log::level RECOVERABLE =
...
const std::unordered_map<ClientBinaryOpcode, std::string>
...
constexpr websocketpp::log::level status_level_to_log_level(StatusLevel level) {
  switch (level) {
...
public:
  ...
  add_channels(const std::vector<ChannelWithoutId> &channels) override;
  ...
  void
  update_parameter_values(const std::vector<Parameter> &parameters) override;

  std::vector<ServiceId>
  add_services(const std::vector<ServiceWithoutId> &services) override;
  ...
  void send_message(ConnHandle client_handle, ChannelId chan_id,
                    uint64_t timestamp, const uint8_t *payload,
                    size_t payload_size) override;

  void broadcast_time(uint64_t timestamp) override;

  void send_service_response(ConnHandle client_handle,
                             const ServiceResponse &response) override;

  void update_connection_graph(const MapOfSets &published_topics,
                               const MapOfSets &subscribed_topics,
                               const MapOfSets &advertised_services) override;

  void send_fetch_asset_response(ConnHandle client_handle,
                                 const FetchAssetResponse &response) override;
...
private:
  ...
  void send_status_and_log_msg(ConnHandle client_handle,
  ...
  void handle_unsubscribe_parameter_updates(const Json &payload,
                                            ConnHandle hdl);
...
private:
  struct ClientInfo {
    ...
    explicit ClientInfo(std::string endpoint_name, std::string user_name,
                        std::string user_id, ConnHandle handle)
        : endpoint_name(std::move(endpoint_name)),
          user_name(std::move(user_name)), user_id(std::move(user_id)),
...
inline Server<ServerConfiguration>::Server(std::string name, LogCallback logger,
  ...
  _handler_callback_queue =
      std::make_unique<CallbackQueue>(_logger, /*numThreads=*/1ul);
}
...
template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::start(const std::string &host,
                                               uint16_t port) {
  if (_server_thread) {
  ...
  if (ec) {
    throw std::runtime_error("Failed to start accepting connections: " +
                             ec.message());
  }
  ...
  if (!_server.is_listening()) {
    throw std::runtime_error("WebSocket server failed to listen on port " +
                             std::to_string(port));
  }
  ...
  if (asioEc) {
    throw std::runtime_error("Failed to resolve the local endpoint: " +
                             ec.message());
  }
...
inline void Server<ServerConfiguration>::stop() {
  ...
  if (!connections.empty()) {
    _server.get_alog().write(APP, "Closing " +
                                      std::to_string(connections.size()) +
                                      " client connection(s)");

    // Iterate over all client connections and start the close connection
    // handshake
  ...
  if (_server_thread) {
    _server.get_alog().write(
        APP, "Waiting for WebSocket server run loop to terminate");
    _server_thread->join();
...
template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::remove_channels(
...
inline void Server<ServerConfiguration>::publish_parameter_values(
  ...
  std::copy_if(parameters.begin(), parameters.end(),
               std::back_inserter(non_empty_parameters), [](const auto &p) {
...
inline void Server<ServerConfiguration>::update_parameter_values(
  ...
  for (const auto &client_param_subscriptions : _client_param_subscriptions) {
    ...
    std::copy_if(parameters.begin(), parameters.end(),
                 std::back_inserter(params_to_send_to_client),
                 [client_param_subscriptions](const Parameter &param) {
                   return client_param_subscriptions.second.find(
                              param.get_name()) !=
                          client_param_subscriptions.second.end();
                 });

    if (!params_to_send_to_client.empty()) {
      publish_parameter_values(client_param_subscriptions.first,
                               params_to_send_to_client, std::nullopt);
    }
  }
}
...
template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::remove_services(
    const std::vector<ServiceId> &service_ids) {
  std::unique_lock<std::shared_mutex> lock(_services_mutex);
  ...
  if (!removed_services.empty()) {
    const auto msg = Json{{"op", "unadvertiseServices"},
...
template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::set_handlers(
    ServerHandlers<ConnHandle> &&handlers) {
  _handlers = handlers;
}

template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::send_message(ConnHandle client_handle,
                                                      ChannelId chan_id,
                                                      uint64_t timestamp,
...
inline void Server<ServerConfiguration>::update_connection_graph(
  ...
  std::copy_if(known_topic_names.begin(), known_topic_names.end(),
               std::back_inserter(removed_topics),
               [&topic_names](const std::string &topic) {
                 return topic_names.find(topic) == topic_names.end();
               });
  std::copy_if(known_service_names.begin(), known_service_names.end(),
  ...
  if (publisher_diff.empty() && subscriber_diff.empty() &&
      services_diff.empty() && removed_topics.empty() &&
      removed_services.empty()) {
    return;
  }
...
inline void Server<ServerConfiguration>::send_fetch_asset_response(
  ...
  const size_t err_msg_size = response.status == FetchAssetStatus::Error
...
template <typename ServerConfiguration>
inline std::string Server<ServerConfiguration>::remote_endpoint_string(
    cobridge_base::ConnHandle client_handle) {
  websocketpp::lib::error_code ec;
  const auto con = _server.get_con_from_hdl(client_handle, ec);
...
inline void Server<ServerConfiguration>::socket_init(ConnHandle hdl) {
  websocketpp::lib::asio::error_code ec;
  _server.get_con_from_hdl(hdl)->get_raw_socket().set_option(
      Tcp::no_delay(true), ec);
  if (ec) {
    _server.get_elog().write(RECOVERABLE,
...
inline bool Server<ServerConfiguration>::validate_connection(ConnHandle hdl) {
  ...
  auto it = std::find(sub_protocols.begin(), sub_protocols.end(),
  ...
  it = std::find(sub_protocols.begin(), sub_protocols.end(),
                 FOXGLOVE_SUB_PROTOCOL);
  if (it != sub_protocols.end()) {
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_login(const Json &payload,
                                               ConnHandle hdl) {
  ...
  _server.get_alog().write(APP, "'" + user_name + " (" + user_id +
                                    ")' is logging in.");

  {
    ...
    if (_clients.size() != 0) {
      for (auto it = _clients.begin(); it != _clients.end();) {
        ...
        _server.get_alog().write(APP, it->second.get_user_info() +
                                          "was kicked by '" + user_name + " (" +
                                          user_id + ")'");
        ++it;
      }
    }
...
template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::handle_connection_opened(
    cobridge_base::ConnHandle hdl) {
  auto con = _server.get_con_from_hdl(hdl);
  const auto endpoint = remote_endpoint_string(hdl);
  _server.get_alog().write(APP, "websocket connection  " + endpoint +
  ...
  if (_clients.size() != 0) {
    ...
    send_json(hdl, {{"op", "login"}, {"userId", ""}, {"username", ""}});
  }
}

template <typename ServerConfiguration>
inline void
Server<ServerConfiguration>::handle_connection_closed(ConnHandle hdl) {
  const auto endpoint = remote_endpoint_string(hdl);
  ...
  this->unsubscribe_params_without_subscriptions(hdl,
                                                 client_subscribed_parameters);

  if (was_subscribed_to_connection_graph) {
    std::unique_lock<std::shared_mutex> lock(_connection_graph_mutex);
    _connection_graph.subscription_count--;
    if (_connection_graph.subscription_count == 0 &&
        _handlers.subscribe_connection_graph_handler) {
      _server.get_alog().write(APP,
...
template <typename ServerConfiguration>
inline void
Server<ServerConfiguration>::unsubscribe_params_without_subscriptions(
    ConnHandle hdl, const std::unordered_set<std::string> &param_names) {
  std::vector<std::string> params_to_unsubscribe;
  {
    std::lock_guard<std::mutex> lock(_client_param_subscriptions_mutex);
    std::copy_if(param_names.begin(), param_names.end(),
                 std::back_inserter(params_to_unsubscribe),
                 [this](const std::string &param_name) {
                   return !is_parameter_subscribed(param_name);
                 });
  }

  if (_handlers.parameter_subscription_handler &&
      !params_to_unsubscribe.empty()) {
    for (const auto &param : params_to_unsubscribe) {
      _server.get_alog().write(APP,
                               "Unsubscribing from parameter '" + param + "'.");
    }

    try {
      _handlers.parameter_subscription_handler(
          params_to_unsubscribe, ParameterSubscriptionOperation::UNSUBSCRIBE,
          hdl);
...
inline bool Server<ServerConfiguration>::is_parameter_subscribed(
  ...
  return std::find_if(_client_param_subscriptions.begin(),
...
template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::handle_message(ConnHandle hdl,
                                                        MessagePtr msg) {
...
template <typename ServerConfiguration>
inline bool Server<ServerConfiguration>::has_capability(
    const std::string &capability) const {
  return std::find(_options.capabilities.begin(), _options.capabilities.end(),
                   capability) != _options.capabilities.end();
}
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_subscribe(const Json &payload,
                                                   ConnHandle hdl) {
  ...
  const auto find_subscription_by_sub_id =
      [](const std::unordered_map<ChannelId, SubscriptionId>
        ...
        return std::find_if(
            ...
            [&sub_id](const auto &mo) { return mo.second == sub_id; });
      };
  ...
  for (const auto &sub : payload.at("subscriptions")) {
    ...
    if (find_subscription_by_sub_id(client_subscriptions_by_channel, sub_id) !=
      ...
      send_status_and_log_msg(hdl, StatusLevel::Error,
    ...
    if (channel_iter == _channels.end()) {
      send_status_and_log_msg(hdl, StatusLevel::Warning,
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_unsubscribe(const Json &payload,
                                                     ConnHandle hdl) {
  std::unordered_map<ChannelId, SubscriptionId> client_subscriptions_by_channel;
  {
    std::shared_lock<std::shared_mutex> clients_lock(_clients_mutex);
  ...
  for (const auto &sub_id_json : payload.at("subscriptionIds")) {
    ...
    const auto &sub =
        find_subscription_by_sub_id(client_subscriptions_by_channel, sub_id);
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_advertise(const Json &payload,
                                                   ConnHandle hdl) {
  std::unique_lock<std::shared_mutex> client_channels_lock(
      _client_channels_mutex);
  auto [client_publications_iter, is_first_publication] =
      _client_channels.emplace(
          hdl, std::unordered_map<ClientChannelId, ClientAdvertisement>());
  ...
  for (const auto &chan : payload.at("channels")) {
    ...
    if (!is_whitelisted(topic, _options.client_topic_whitelist_patterns)) {
      send_status_and_log_msg(hdl, StatusLevel::Error,
                              "Can't advertise channel " +
                                  std::to_string(channel_id) + ", topic '" +
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_unadvertise(const Json &payload,
                                                     ConnHandle hdl) {
  std::unique_lock<std::shared_mutex> clientChannelsLock(
      _client_channels_mutex);
  auto client_publications_iter = _client_channels.find(hdl);
  if (client_publications_iter == _client_channels.end()) {
    send_status_and_log_msg(hdl, StatusLevel::Error,
                            "Client has no advertised channels");
    return;
  }
  ...
  for (const auto &chan_id_json : payload.at("channelIds")) {
    ...
    const auto advertised_channel_iter =
        client_info.advertised_channels.find(channel_id);
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_get_parameters(const Json &payload,
                                                        ConnHandle hdl) {
  const auto param_names =
      payload.at("parameterNames").get<std::vector<std::string>>();
  const auto request_id =
      payload.find("id") == payload.end()
          ? std::nullopt
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_set_parameters(const Json &payload,
                                                        ConnHandle hdl) {
  const auto parameters =
      payload.at("parameters").get<std::vector<Parameter>>();
  const auto request_id =
      payload.find("id") == payload.end()
          ? std::nullopt
          : std::optional<std::string>(payload["id"].get<std::string>());
...
void Server<ServerConfiguration>::handle_subscribe_parameter_updates(
  ...
  std::vector<std::string> params_to_subscribe;
  {
    ...
    std::lock_guard<std::mutex> lock(_client_param_subscriptions_mutex);
    std::copy_if(param_names.begin(), param_names.end(),
                 std::back_inserter(params_to_subscribe),
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_subscribe_connection_graph(
    ConnHandle hdl) {
  ...
  Json::array_t published_topics_json, subscribed_topics_json,
      advertised_services_json;
  {
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_unsubscribe_connection_graph(
    ConnHandle hdl) {
  ...
  if (client_was_subscribed) {
    bool unsubscribe_from_connection_graph = false;
    {
      std::unique_lock<std::shared_mutex> lock(_connection_graph_mutex);
      _connection_graph.subscription_count--;
      unsubscribe_from_connection_graph =
          _connection_graph.subscription_count == 0;
    }
...
template <typename ServerConfiguration>
void Server<ServerConfiguration>::handle_fetch_asset(const Json &payload,
                                                     ConnHandle hdl) {
...
template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::handle_text_message(ConnHandle hdl,
                                                             MessagePtr msg) {
  ...
  try {
    ...
    const std::string postfix = " (op: " + op + ")";
    send_status_and_log_msg(hdl, StatusLevel::Error,
                            "Failed to execute handler" + postfix);
  }
}

template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::handle_binary_message(ConnHandle hdl,
                                                               MessagePtr msg) {
  ...
  const auto required_capability_iter =
      CAPABILITY_BY_CLIENT_BINARY_OPERATION.find(op);
  ...
  case ClientBinaryOpcode::MESSAGE_DATA: {
    ...
    const auto timestamp =
        std::chrono::duration_cast<std::chrono::nanoseconds>(
            ...
            .count();
    const ClientChannelId channel_id =
        *reinterpret_cast<const ClientChannelId *>(data + 1);
  ...
  case ClientBinaryOpcode::SERVICE_CALL_REQUEST: {
    ...
    if (length < request.size()) {
      send_status_and_log_msg(hdl, StatusLevel::Error,
                              "Invalid service call request length " +
                                  std::to_string(length));
      return;
    }
  ...
  default: {
    send_status_and_log_msg(hdl, StatusLevel::Error,
                            "Unrecognized client opcode " +
                                std::to_string(uint8_t(op)));
  } break;
  }
}
...
inline void Server<ServerConfiguration>::send_status_and_log_msg(
  ...
  auto logger =
      level == StatusLevel::Info ? _server.get_alog() : _server.get_elog();
  logger.write(logLevel, logMessage);

  send_json(client_handle, Json{
                               {"op", "status"},
...
template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::send_json(ConnHandle hdl,
                                                   Json &&payload) {
  try {
...
template <typename ServerConfiguration>
inline void
Server<ServerConfiguration>::send_raw_json(ConnHandle hdl,
...
template <typename ServerConfiguration>
inline void Server<ServerConfiguration>::send_binary(ConnHandle hdl,
                                                     const uint8_t *payload,
                                                     size_t payload_size) {
  try {
