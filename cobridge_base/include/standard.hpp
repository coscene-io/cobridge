...
private:
  ...
  struct _Storage : _Storage_base
  {
    ...
    _Storage(const _Up& _value) : M_value(_value) {}
...
public:
  ...
  constexpr string_view(const char* str) : data_(str), size_(str ? strlen_constexpr(str) : 0) {}
  string_view(const std::string& str) : data_(str.data()), size_(str.size()) {}
  constexpr string_view(const char* data, std::size_t size) : data_(data), size_(size) {}
